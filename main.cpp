...
int main()
{
    ...
    for (int i = 0; i < 40; i++) {
        ...
        if (i < 20) {
            ...
            line[i].setRotation(sf::degrees(90));
            if (i < 30)
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (i < 2)
            strel[i].setPosition(sf::Vector2f(x0, 0));
        else
            strel[i].setPosition(sf::Vector2f(W, y0));
    }
    strel[0].setRotation(sf::degrees(25));
    strel[1].setRotation(sf::degrees(-25));
    strel[2].setRotation(sf::degrees(60));
    strel[3].setRotation(sf::degrees(-250));

    int anim = 0;

    while (window.isOpen())
    {
        // Обработка событий для SFML 3.0
